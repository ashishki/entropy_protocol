...
| TASK-AF-004 | F-4 | P0 | Open | `docs/audit/AUDIT_v1.md` | P4 signal algorithm undefined. Phase 0 exit criterion, Phase 1 OOS spanning, and Phase 2 IC correlation requirement all depend on a P4 algorithm that is nowhere specified. | PROTOCOL_SPEC.md Section E contains a complete P4 specification: inputs, features, thresholds or model class, calibration method, and the assignment rule for "trending / mean-reverting / stress." A second developer can independently reproduce the historical label series from the same raw data. | — |
| TASK-AF-005 | F-5 | P0 | Open | `docs/audit/AUDIT_v1.md` | IC_long assumption is load-bearing and unvalidated; no suspect threshold exists unlike IC_short. At ρ_avg=0.40, effective FLAM gross ≈ 0.25, below K1. | CHARTER.md and PROTOCOL_SPEC.md define: (a) a suspect threshold for IC_long analogous to IC_short's >0.04 rule, (b) a haircut rule if walk-forward IC_long exceeds the threshold, (c) a literature citation or prior justification for the 0.03–0.05 range, (d) FLAM BR_eff calculation that accounts for skill correlation (using N_eff correction). | — |
