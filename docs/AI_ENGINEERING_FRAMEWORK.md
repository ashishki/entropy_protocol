...
3. `PROTOCOL_SPEC.md` (current specification)
4. `CHARTER.md` (strategic constraints)
5. `EVOLUTION.md` (design history — prevents re-litigation of closed questions)
...
1. `GLOSSARY.md` (key terms)
2. Relevant `PROTOCOL_SPEC.md` section for the component being built
...
```
1. docs/GLOSSARY.md
2. docs/PROTOCOL_SPEC.md (or relevant sections)
3. docs/CHARTER.md (Non-Negotiables section minimum)
```
...
```
1. docs/GLOSSARY.md
2. Specific PROTOCOL_SPEC section relevant to the component
```
...
Current date: [DATE]
Active documents: PROTOCOL_SPEC.md v1.1, CHARTER.md v5.0
Frozen constraints: NN-1 through NN-6 (see PROTOCOL_SPEC Section B)
```
...
If you need to reference a specific historical decision, load `EVOLUTION.md` instead.
...
`PROTOCOL_SPEC.md` defines what the system must do. It is the authoritative source for:
...
2. If creating, determine which category it belongs to (core / process / audience / archive)
3. Add it to README.md document map

Do not create documents that duplicate content from PROTOCOL_SPEC or CHARTER. Reference those documents instead.
...
2. `PROTOCOL_SPEC.md` (full)
3. `EVOLUTION.md` (design history)
