...
- One shared tracing module: `entropy/tracing.py` with a single `get_tracer()` function.
...
2. An ADR in `docs/adr/` documenting the bottleneck, selected language, affected modules, CI/toolchain changes, packaging strategy, ownership burden, and rollback plan.
3. Updates to `docs/ARCHITECTURE.md`, CI, and `docs/tasks.md`.
...
| Network egress | No external network egress in Phase 0 v1. Data ingestion uses local fixture files only. Any future provider adapter must be declared in docs/ARCHITECTURE.md §External Integrations before egress is opened. |
...
|----------------|------------------|
| Implementation rules | `docs/IMPLEMENTATION_CONTRACT.md` |
| Technical architecture | `docs/ARCHITECTURE.md` |
| Feature scope | `docs/spec.md` |
...
|---------------|-------------|-------------|
| `docs/DECISION_LOG.md` | Find decisions and their canonical source | Override a canonical document |
| `docs/IMPLEMENTATION_JOURNAL.md` | Recover recent session context and handoff notes | Authorize implementation changes |
| `docs/EVIDENCE_INDEX.md` | Locate tests, reports, and proof artifacts | Declare a finding closed without verification |
...
- The proposed change modifies a file listed in `docs/EVIDENCE_INDEX.md`.

Minimum lookup set for these cases: relevant rows in `docs/DECISION_LOG.md`, latest entries in `docs/IMPLEMENTATION_JOURNAL.md`, and relevant rows in `docs/EVIDENCE_INDEX.md`.

### Write-Side Rules

- Append to `docs/DECISION_LOG.md` when a technical decision affects architecture, runtime boundaries, data model shape, provider strategy, migration strategy, or task sequencing. Every entry must point to a canonical source or ADR.
- Append to `docs/IMPLEMENTATION_JOURNAL.md` after each implementation session that changes code, tests, migrations, CI, or generated project governance files. Include date, task ID, files changed, tests run, and unresolved follow-ups.
- Update `docs/EVIDENCE_INDEX.md` when adding or changing a test, report, migration check, phase-gate artifact, or audit evidence artifact used by acceptance criteria.
...
| Protocol spec | `docs/core/PROTOCOL_SPEC.md` | Protocol authority — canonical research governance rules |
| Charter | `docs/core/CHARTER.md` | Strategic authority — frozen non-negotiables and kill criteria |
...
2. docs/core/PROTOCOL_SPEC.md — overrides architecture and spec for protocol-level rules
3. docs/adr/ — overrides architecture and spec when a formal decision was made
4. docs/ARCHITECTURE.md — overrides spec for technical design
5. docs/spec.md — overrides tasks for feature scope
6. docs/tasks.md — overrides CODEX_PROMPT for task-level details
