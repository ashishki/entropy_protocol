...
The main architecture issue is documentation freshness: `docs/audit/REVIEW_REPORT.md`
...
|----|----------|-------|-------|----------------|--------------------|
| PSR-001 | P1 | strategist | `docs/audit/REVIEW_REPORT.md`, `docs/audit/AUDIT_INDEX.md` | Canonical consolidated review is stale after T23/T24. Future agents may rely on Phase 8 stop-ship state instead of current T24 foundation state. | Write a Phase 0 consolidated review or update `REVIEW_REPORT.md` to point to `PHASE0_FOUNDATION_REVIEW.md` and baseline 135. |
| PSR-002 | P1 | human | `docs/audit/PHASE0_FOUNDATION_REVIEW.md`, `docs/CODEX_PROMPT.md` | Phase 0 implementation is complete but not approved. Next implementation should not start until the human chooses closure/hardening/Phase 1 direction. | Record a human Phase 0 foundation decision: accept foundation, request fixes, or reject/reshape. |
| PSR-003 | P2 | codex | `docs/tasks.md`, `docs/EVIDENCE_INDEX.md` | Next-stage tasks are not yet represented as executable task graph entries. | After human decision, add a Phase 0 hardening or Phase 1 planning task block with acceptance tests/evidence. |
| PSR-004 | P2 | strategist | `docs/ARCHITECTURE.md`, `docs/spec.md` | Architecture/spec still describe some future surfaces at a high level; post-T24 reality should be summarized before the next phase. | Add a short "Phase 0 Foundation Status" section or update existing component notes. |
| PSR-005 | P2 | human/strategist | `docs/adr/`, `docs/core/*` | Formula and evidence debt remains intentionally open. | Decide whether to open ADR/review packets for Harvey-Liu, Sharpe CI, purge/embargo, N_eff/K3, F-30, F-31 before Phase 1. |
...
- `docs/audit/REVIEW_REPORT.md` — current Phase 0 consolidated review.
- `docs/audit/NEXT_PHASE_PLAN.md` — next-stage plan from this strategy review.
- `docs/tasks.md` — add Phase 0 closure/hardening or Phase 1 planning tasks
  after human decision.
- `docs/CODEX_PROMPT.md` — set next task to the human decision or first approved
  hardening task.
- `docs/EVIDENCE_INDEX.md` — add strategy review and future hardening evidence.
