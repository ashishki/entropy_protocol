# Trader Risk Audit Report - Real Open DEX Swaps Reviewed Copy

Reviewed status: T120 manual review complete.
...
- source rows are Uniswap V2 WETH/USDC pair-level swap logs, not one trader
  account ledger;
- `account_id` is a safe pair-scope label, not a wallet or customer id;
- side is derived relative to WETH and does not represent a verified trader
  intent;
