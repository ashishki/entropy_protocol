...
**Filename:** `products/entropy-core/docs/architecture/workflow_ai_development.md`
**Version:** 1.5
**Date:** 2026-03-16
...
This document defines development workflow policies for the Entropy Protocol, with a focus on governance, audit integration, and AI-assisted development constraints. It complements `products/entropy-core/docs/architecture/AI_ENGINEERING_FRAMEWORK.md`, which covers AI model roles and context-loading protocols.
...
All audit artifacts live in `products/entropy-core/docs/audit/`. No audit output is placed in project root, `products/entropy-core/docs/archive/`, or any other location.

Canonical artifact filenames are defined in `products/entropy-core/docs/audit/AUDIT_INDEX.md`. Versioned snapshots use the naming convention `YYYY-MM-DD_phase<N>_<ARTIFACT>_v<N>.md`.

No audit artifacts should be placed outside `products/entropy-core/docs/audit/` for any reason, including convenience or staging.
...
See `products/entropy-core/docs/audit/AUDIT_INDEX.md` for the complete naming specification.
...
| **In Progress** | Remediation underway; corresponding task exists in `products/entropy-core/docs/tasks.md`. |
...
1. Have a corresponding entry in `products/entropy-core/docs/tasks.md` (Audit Findings Backlog section) within 48 hours of the REVIEW_REPORT being marked Draft.
2. Link to the source finding in `products/entropy-core/docs/audit/REVIEW_REPORT.md` (or the versioned snapshot that introduced it).
...
4. Reference an ADR (Architectural Decision Record) or PR if the remediation involves a spec change. If no ADR mechanism is in place, a dated comment in `products/entropy-core/docs/core/EVOLUTION.md` serves as the ADR reference.
...
- In addition to baseline findings, remediation planning must explicitly include `TASK-AF-022` through `TASK-AF-032` from `products/entropy-core/docs/tasks.md` (sourced from `products/entropy-core/docs/audit/REVIEW_REPORT.md`, Cycle 1).
...
**Policy statement:** No change to `products/entropy-core/docs/core/PROTOCOL_SPEC.md`, `products/entropy-core/docs/core/CHARTER.md`, or `products/entropy-core/docs/core/GLOSSARY.md` is accepted without one of the following:

- A reference to an open audit finding (by finding ID, e.g., "Resolves F-1") that the change addresses, OR
- A new ADR entry in `products/entropy-core/docs/core/EVOLUTION.md` (for architectural decisions) or an equivalent dated record
...
RDL boundary is phase-sliced and machine-checkable (see `products/entropy-core/docs/core/PROTOCOL_SPEC.md` Section E):
...
The full audit pipeline is defined in `products/entropy-core/docs/audit/review_pipeline.md`. Key governance requirements:
...
*See also: `products/entropy-core/docs/architecture/AI_ENGINEERING_FRAMEWORK.md` (AI model roles, context loading), `products/entropy-core/docs/audit/review_pipeline.md` (pipeline execution), `products/entropy-core/docs/audit/AUDIT_INDEX.md` (artifact registry)*
