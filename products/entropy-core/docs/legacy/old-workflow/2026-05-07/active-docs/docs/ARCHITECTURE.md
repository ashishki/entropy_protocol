...
| Planning | OFF | No LLM-generated plans in the application. Task sequencing is encoded in products/entropy-core/docs/tasks.md and executed by human-directed orchestration. |
...
|-----------|--------|---------|
| Trial Registry | src/entropy/registry/ | Preregistration of trial specs before data examination; append-only; family tagging; hash locking |
| SimBroker | src/entropy/simbroker/ | Deterministic execution simulation: fee model, fill engine, borrow/funding hooks, fill logs |
| Walk-Forward Harness | src/entropy/walkforward/ | IS/OOS splitting with embargo bands; leakage detection; walk-forward runner |
| P&L Attribution Engine | src/entropy/attribution/ | Four-stream P&L decomposition; net Sharpe computation; DrawdownRecord |
| Governance State Machine | src/entropy/governance/ | P1 drawdown circuit breaker; P3 correlation trigger; state transitions; event logging |
| Data Pipeline | src/entropy/data/ | OHLCV ingestion; DataProvider abstraction; local Parquet store; data quality checks |
| Statistical Analysis | src/entropy/stats/ | Sharpe CI (CI-SR-ACF-v1); Harvey-Liu deflation stub; N_eff estimator stub |
| Phase Gate Evidence | src/entropy/evidence/ | Evaluation report generator; leakage audit evidence collection; EVIDENCE_INDEX updater |
| Database Layer | src/entropy/db/ | SQLAlchemy 2.x models; Alembic migrations; PostgreSQL 16 tables |
| CLI | src/entropy/cli/ | Typer/rich CLI entry point; commands for registry, ingestion, walk-forward, reporting |
| Models | src/entropy/models/ | Pydantic v2 domain models: market, registry, performance |
| Hashing | src/entropy/hashing/ | Deterministic SHA-256 hashing for datasets, runs, policies |
| Experiment Readiness Gate (ERG) | src/entropy/registry/gate.py | Deterministic pre-admission validation for registered trials; no DB writes |
| Research Firewall | products/entropy-core/docs/governance/research_firewall.md; src/entropy/registry/ | Governance boundary preventing discovery artifacts from becoming admissible evaluation without preregistration |
| Research Portfolio Monitor (RPM) | products/entropy-core/docs/governance/research_portfolio_monitor.md | Read-only governance dashboard specification; no runtime module in Phase 0 implementation yet |
| Protocol Governor | products/entropy-core/docs/governance/governor.md; src/entropy/governance/ | Governance gate for system-level changes and phase approvals |
| Research Discovery Layer (RDL) | products/entropy-core/docs/architecture/research_discovery_layer.md | Dormant/scaffolding-only through Phase 0-1; no application runtime module until an approved future phase |
...
5. **Generate report** — `entropy report generate --trial-id <id>` fetches the registry spec, dataset hash, code hash, and policy hash from the DB. It renders a reproducible Markdown evaluation report. The leakage audit evidence is appended to `docs/EVIDENCE_INDEX.md` inside the Entropy Core workspace. The Phase 0 gate report is produced when all T01–T24 tasks have passing tests.
...
2. A written ADR in `products/entropy-core/docs/adr/` explaining why Python/Polars/DuckDB are insufficient, why the chosen language is appropriate, affected modules, CI/toolchain impact, packaging strategy, and rollback plan.
3. Updates to `products/entropy-core/docs/ARCHITECTURE.md`, `products/entropy-core/docs/IMPLEMENTATION_CONTRACT.md`, CI, and `products/entropy-core/docs/tasks.md`.
...
- **Tracing:** OpenTelemetry noop tracer in v1. A single shared tracing module at `src/entropy/tracing.py` exposes `get_tracer()`. All code that creates spans imports from this module. No inline noop spans in individual files. No external collector in v1.
- **Metrics:** No external metrics collection in v1. Counter/histogram stubs in `src/entropy/metrics.py` for future activation.
...
├── docs/                             # Portfolio-level direction only
└── products/
    └── entropy-core/
        ├── docs/                     # Core architecture, governance, audit, archive
        ├── src/entropy/              # Core Python package
        │   ├── cli.py
        │   ├── tracing.py
        │   ├── metrics.py
        │   ├── models/
        ...
        │   ├── stats/
        │   └── evidence/
        ├── migrations/               # Alembic migration config and versions
        ├── tests/                    # Smoke, unit, and integration tests
        ├── pyproject.toml            # Core package and tool config
        ├── pyrightconfig.json
        ├── .env.example
        └── .gitignore
```
...
- **Canonical authority:** `products/entropy-core/docs/ARCHITECTURE.md`, `products/entropy-core/docs/IMPLEMENTATION_CONTRACT.md`, `products/entropy-core/docs/tasks.md`, `products/entropy-core/docs/CODEX_PROMPT.md`, review reports, code, tests.
- **Retrieval aids:** `products/entropy-core/docs/DECISION_LOG.md`, `products/entropy-core/docs/IMPLEMENTATION_JOURNAL.md`, `products/entropy-core/docs/EVIDENCE_INDEX.md` — these summarize and point; they do not overrule canonical files.
- **Session handoff:** `products/entropy-core/docs/CODEX_PROMPT.md` is the single source of truth for session state. Every agent reads it before starting work.
- **Phase gate evidence:** `products/entropy-core/docs/EVIDENCE_INDEX.md` tracks proof artifacts for heavy tasks (T16, T17, T21, T22) and phase gates. Evidence is required before a phase gate can be marked PASSED.
