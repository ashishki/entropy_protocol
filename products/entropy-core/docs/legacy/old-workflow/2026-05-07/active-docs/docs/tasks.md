...
`products/entropy-core/docs/archive/session_state/tasks_full_2026-05-06.md`.
...
|---|---|---|
| P1D-001 Long-Only Implementation Contract | [x] | `src/entropy/baseline/implementation.py`; `tests/unit/test_phase1d_implementation_contract.py`; D-051 |
| P1E-001 Bounded Long-Only Baseline Logic | [x] | `src/entropy/baseline/bounded.py`; `tests/unit/test_phase1e_bounded_baseline.py`; D-052 |
| P1F-001/P1F-002 Baseline Registration Integration | [x] | `src/entropy/baseline/registration.py`; `tests/unit/test_phase1f_registration.py`; D-053 |
| P1G-001 Evaluation Configuration Contract | [x] | `src/entropy/baseline/evaluation.py`; `tests/unit/test_phase1g_evaluation_config.py`; D-054 |
| P1H-001 First Archive-Only Governed Evaluation Run | [x] | `src/entropy/baseline/governed.py`; `tests/unit/test_phase1h_governed_evaluation.py`; D-054 |
| P1I-001 Evaluation Report Assembly | [x] | `src/entropy/baseline/report.py`; `tests/unit/test_phase1i_j_k_packets.py`; D-055/D-056 |
| P1J-HUMAN-001/P1J-HUMAN-002 Research Decision And Holdout Gate | [x] | `src/entropy/baseline/decision.py`; D-055 |
| P1K-001/P1K-HUMAN-001 Archive-Only Closure | [x] | `src/entropy/baseline/decision.py`; D-055 |
| DK-REVIEW-001 Full D-K Deep Review And Fix Closure | [x] | `products/entropy-core/docs/audit/REVIEW_REPORT.md`; D-056 |
...
Acceptance criteria:
- Decision is recorded in `products/entropy-core/docs/DECISION_LOG.md`.
- `products/entropy-core/docs/CODEX_PROMPT.md`, `products/entropy-core/docs/audit/AUDIT_INDEX.md`, and this task graph
...
Strategist inputs:
- `products/entropy-core/docs/audit/POST_DK_STRATEGY_REVIEW.md`
- `products/entropy-core/docs/audit/NEXT_PHASE_PLAN.md`

---

## Proposed: P0C Phase 0 Exit Evidence And D-K Admission Planning

Status:
Proposed only. Do not start until SO-DK-001 accepts D-K fix closure and opens
this block.

Objective:
Create a compact, auditable bridge from D-K fix closure to the next admissible
development block by synchronizing current-state docs, producing a Phase 0 exit
evidence matrix, and designing D-K admission into the main control plane without
claim escalation.
...
Forbidden:
No OOS/performance claims, holdout reads, live feeds, broker integration,
capital deployment, production/capital-ready labels, Growth/RDL/RBE activation,
Phase 2 overlay, shorts, treasury, or runtime escalation.
