...
- **Admissibility references:** Discovery work in RDL is governed by the Research Firewall, Experiment Readiness Gate, and Hypothesis Families registry. No AI-generated research object becomes admissible to the main evaluation protocol without explicit human registration in the Trial Registry, family assignment, and readiness review.
...
- **Experiment Readiness Gate:** every new experiment must define falsifiability, metric, baseline, leakage review, family, and Trial Registry readiness before evaluation.
...
- Full specification: `products/entropy-core/docs/governance/governor.md`.
...
**Research Knowledge Graph (metadata-only)**
- Purpose: Maintain long-term structured memory across `Hypothesis`, `Feature`, `Experiment`, `Result`, `Dataset`, and `Regime` objects.
- Allowed use: lineage tracking, duplicate-detection support, failed-idea memory, and AI-assisted research analysis.
- Prohibited interpretation: the graph is not a scoring engine, not a signal engine, and not a substitute for Trial Registry or walk-forward evaluation.
