...
These documents explain why current constraints exist. They are not superseded in the sense of being wrong; they are superseded in the sense that their conclusions have been incorporated into `products/entropy-core/docs/core/CHARTER.md` and `products/entropy-core/docs/core/PROTOCOL_SPEC.md`.

**Do not use archived documents as context for implementation or specification work.** Use `products/entropy-core/docs/core/EVOLUTION.md` instead, which summarizes the key decisions and their rationale in a single navigable document.
...
`strategic_architecture_review_v4.md` is the most important archive document. It contains the specific numerical derivations and adversarial challenges that produced the three mandatory corrections in `products/entropy-core/docs/core/CHARTER.md`. If a future review challenges the FLAM derivation, stop-loss cost model, or K4 definition, read v4 Section 1.3, 4.3, and 5.3 respectively before responding.
