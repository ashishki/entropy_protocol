# Audit Index - Entropy Core

Append-only. One row per reset-era review cycle.

## Review Schedule

| Cycle | Phase | Date | Scope | Stop-Ship | P0 | P1 | P2 |
|-------|-------|------|-------|-----------|----|----|----|
| PHASE1-RESET | 1 | 2026-05-07 | T01-T03 Reset Foundation | 0 | 0 | 0 | 0 |
| PHASE2-GOVERNANCE | 2 | 2026-05-07 | T04-T07 Governance Integrity | 0 | 0 | 0 | 0 |
| PHASE3-EVALUATION | 3 | 2026-05-07 | T08-T11 Evaluation Safety | 0 | 0 | 0 | 0 |
| RESET-CLOSURE | 4 | 2026-05-07 | T01-T14 Reset Closure | 0 | 0 | 0 | 0 |
| FIRST-RESEARCH-PACKET | 5 | 2026-05-07 | T15-T19 First Research Evidence Packet | 0 | 0 | 0 | 0 |
| ARCHIVE-EVIDENCE-EXPANSION | 6 | 2026-05-07 | T20-T24 Archive Evidence Expansion | 0 | 0 | 0 | 0 |
| ARCHIVE-REPRODUCIBILITY-HARDENING | 7 | 2026-05-08 | T25-T29 Archive Reproducibility Hardening | 0 | 0 | 0 | 0 |
| PHASE-GATE-READINESS-REVIEW | 8 | 2026-05-09 | T30-T34 Phase-Gate Readiness Review | 0 | 0 | 0 | 0 |
| HOLDOUT-ACCESS-PROTOCOL | 9 | 2026-05-09 | T35-T39 Holdout Access Protocol | 0 | 0 | 0 | 0 |
| HOLDOUT-APPROVAL-DECISION | 10 | 2026-05-09 | T40-T45 Holdout Approval Decision Packet | 0 | 0 | 0 | 0 |
| LIVE-FEED-READINESS | 11 | 2026-05-09 | T46-T50 Live-Feed Dry Run Readiness | 0 | 0 | 0 | 0 |
| BROKER-SANDBOX-READINESS | 12 | 2026-05-09 | T51-T56 Broker Sandbox and Execution Risk Audit | 0 | 0 | 0 | 0 |
| PRODUCT-HYPOTHESIS-CONFIRMATION-DECISION | 13 | 2026-05-09 | T57-T62 Product Hypothesis Confirmation Decision | 0 | 0 | 0 | 0 |
| LOCAL-REPLAY-EXTENSION | 14 | 2026-05-09 | T63-T68 Local Broker Sandbox No-Capital Replay Extension | 0 | 0 | 0 | 0 |
| ARTIFACT-SUPPORT | 15 | 2026-05-12 | T69-T74 Artifact Support Mode | 0 | 0 | 0 | 0 |
| EXECUTABLE-ARTIFACT-VALIDATION | 16 | 2026-05-14 | T75-T78 Executable Artifact Validation | 0 | 0 | 0 | 1 |
| ARTIFACT-REGISTRY | 17 | 2026-05-14 | T79-T82 Artifact Registry | 0 | 0 | 0 | 1 |
| REPRODUCIBILITY-RUNNER | 18 | 2026-05-14 | T83-T86 Reproducibility Runner | 0 | 0 | 0 | 1 |
| EVIDENCE-PIPELINE | 19 | 2026-05-14 | T87-T90 Evidence Pipeline | 0 | 0 | 0 | 1 |
| PRODUCT-BRIDGE-PROFILES | 20 | 2026-05-14 | T91-T94 Product Bridge Profiles | 0 | 0 | 0 | 1 |
| ARTIFACT-GOVERNANCE-STATE-MACHINE | 21 | 2026-05-14 | T95-T98 Artifact Governance State Machine | 0 | 0 | 0 | 1 |
| RESEARCH-ARTIFACT-INTEGRATION | 22 | 2026-05-14 | T99-T102 Research Artifact Integration | 0 | 0 | 0 | 1 |
| STORAGE-AND-AUDIT-BACKEND | 23 | 2026-05-14 | T103-T106 Storage And Audit Backend | 0 | 0 | 0 | 1 |
| INTERNAL-API-JOB-BOUNDARY | 24 | 2026-05-14 | T107-T110 Internal API And Job Boundary | 0 | 0 | 0 | 1 |
| CAF-DECISION-PRIMITIVES | 25 | 2026-05-14 | T111-T114 CAF Decision Primitives | 0 | 0 | 0 | 1 |
| ENTERPRISE-AUDIT-READINESS | 26 | 2026-05-14 | T115-T118 Enterprise Audit Readiness | 0 | 0 | 0 | 1 |
| CORE-V1-PRODUCTIZATION | 27 | 2026-05-14 | T119-T122 Core V1 Productization | 0 | 0 | 0 | 1 |

## Archive

| Cycle | File | Phase | Health |
|-------|------|-------|--------|
| PHASE1-RESET | `docs/audit/PHASE1_REVIEW.md` | 1 | PASS |
| PHASE2-GOVERNANCE | `docs/audit/PHASE2_REVIEW.md` | 2 | PASS |
| PHASE3-EVALUATION | `docs/audit/PHASE3_REVIEW.md` | 3 | PASS |
| RESET-CLOSURE | `docs/audit/RESET_REVIEW.md` | 4 | PASS |
| FIRST-RESEARCH-PACKET | `docs/audit/FIRST_RESEARCH_PACKET_REVIEW.md` | 5 | PASS |
| ARCHIVE-EVIDENCE-EXPANSION | `docs/audit/ARCHIVE_EVIDENCE_EXPANSION_REVIEW.md` | 6 | PASS |
| ARCHIVE-REPRODUCIBILITY-HARDENING | `docs/audit/ARCHIVE_REPRODUCIBILITY_REVIEW.md` | 7 | PASS |
| PHASE-GATE-READINESS-REVIEW | `docs/audit/PHASE_GATE_READINESS_REVIEW.md` | 8 | PASS |
| HOLDOUT-ACCESS-PROTOCOL | `docs/audit/HOLDOUT_ACCESS_PROTOCOL_REVIEW.md` | 9 | PASS |
| HOLDOUT-APPROVAL-DECISION | `docs/audit/HOLDOUT_APPROVAL_DECISION_REVIEW.md` | 10 | PASS |
| LIVE-FEED-READINESS | `docs/audit/LIVE_FEED_READINESS_REVIEW.md` | 11 | PASS |
| BROKER-SANDBOX-READINESS | `docs/audit/BROKER_SANDBOX_READINESS_REVIEW.md` | 12 | PASS |
| PRODUCT-HYPOTHESIS-CONFIRMATION-DECISION | `docs/audit/PRODUCT_HYPOTHESIS_CONFIRMATION_DECISION_REVIEW.md` | 13 | PASS |
| LOCAL-REPLAY-EXTENSION | `docs/audit/LOCAL_REPLAY_EXTENSION_REVIEW.md` | 14 | PASS |
| ARTIFACT-SUPPORT | `docs/audit/ARTIFACT_SUPPORT_REVIEW.md` | 15 | PASS |
| EXECUTABLE-ARTIFACT-VALIDATION | `docs/audit/EXECUTABLE_ARTIFACT_VALIDATION_REVIEW.md` | 16 | PASS |
| ARTIFACT-REGISTRY | `docs/audit/ARTIFACT_REGISTRY_REVIEW.md` | 17 | PASS |
| REPRODUCIBILITY-RUNNER | `docs/audit/REPRODUCIBILITY_RUNNER_REVIEW.md` | 18 | PASS |
| EVIDENCE-PIPELINE | `docs/audit/EVIDENCE_PIPELINE_REVIEW.md` | 19 | PASS |
| PRODUCT-BRIDGE-PROFILES | `docs/audit/PRODUCT_BRIDGE_PROFILE_REVIEW.md` | 20 | PASS |
| ARTIFACT-GOVERNANCE-STATE-MACHINE | `docs/audit/ARTIFACT_GOVERNANCE_STATE_MACHINE_REVIEW.md` | 21 | PASS |
| RESEARCH-ARTIFACT-INTEGRATION | `docs/audit/RESEARCH_ARTIFACT_INTEGRATION_REVIEW.md` | 22 | PASS |
| STORAGE-AND-AUDIT-BACKEND | `docs/audit/STORAGE_AND_AUDIT_BACKEND_REVIEW.md` | 23 | PASS |
| INTERNAL-API-JOB-BOUNDARY | `docs/audit/INTERNAL_API_JOB_BOUNDARY_REVIEW.md` | 24 | PASS |
| CAF-DECISION-PRIMITIVES | `docs/audit/CAF_DECISION_PRIMITIVES_REVIEW.md` | 25 | PASS |
| ENTERPRISE-AUDIT-READINESS | `docs/audit/ENTERPRISE_AUDIT_READINESS_REVIEW.md` | 26 | PASS |
| CORE-V1-PRODUCTIZATION | `docs/audit/CORE_V1_PRODUCTIZATION_REVIEW.md` | 27 | PASS |

## Notes

- Pre-reset audit files live under `docs/legacy/old-workflow/2026-05-07/` or existing archive directories.
- Run Phase 1 validation before T01 and write `docs/audit/PHASE1_AUDIT.md`.
