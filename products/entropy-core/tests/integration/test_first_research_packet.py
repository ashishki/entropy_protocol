"""First research packet integration contract tests."""
...
CANDIDATE_PACKET_DOC = PROJECT_ROOT / "docs" / "research" / "first-packet" / "CANDIDATE_PACKET.md"
DATASET_MANIFEST_DOC = PROJECT_ROOT / "docs" / "research" / "first-packet" / "DATASET_MANIFEST.md"
RESEARCH_EVIDENCE_PACKET_DOC = (
    PROJECT_ROOT / "docs" / "research" / "first-packet" / "RESEARCH_EVIDENCE_PACKET.md"
)


def test_candidate_packet_records_registration_requirements() -> None:
    packet = build_first_research_candidate_packet()
    document = CANDIDATE_PACKET_DOC.read_text(encoding="utf-8")
    ...
    assert packet.hypothesis_family == "Volatility Compression"
...
def test_candidate_packet_serializes_deterministically() -> None:
    packet = build_first_research_candidate_packet()
    ...
    assert packet.to_markdown() == CANDIDATE_PACKET_DOC.read_text(encoding="utf-8")
...
def test_candidate_packet_rejects_claim_and_live_surfaces(requested_surface: str) -> None:
    base = build_first_research_candidate_packet()
...
def test_archive_dataset_manifest_hash_is_deterministic(tmp_path: Path) -> None:
    ...
    first_binding = ArchiveDatasetBinding(
        ...
        path="archive/btc/formation.parquet",
        dataset_hash=first_hash,
        row_count=2,
        role="formation",
    )
    second_binding = ArchiveDatasetBinding(
        dataset_id="btc-archive-evaluation-probe",
        path="archive/btc/evaluation_probe.parquet",
    ...
    manifest_a = build_archive_dataset_manifest(
        candidate_id=build_first_research_candidate_packet().candidate_id,
        dataset_bindings=(second_binding, first_binding),
        formation_scope="first 60 percent archive fixture rows",
        evaluation_scope="remaining archive fixture rows; no holdout",
    )
...
def test_archive_dataset_manifest_excludes_holdout() -> None:
    document = DATASET_MANIFEST_DOC.read_text(encoding="utf-8")
    ...
    with pytest.raises(DatasetManifestError, match="Holdout paths must be excluded"):
        build_archive_dataset_manifest(
            ...
            holdout_paths=("holdout/btc.parquet",),
        )
    with pytest.raises(ValidationError, match="Holdout path is forbidden"):
        ArchiveDatasetBinding(
            dataset_id="btc-holdout",
            path="holdout/btc.parquet",
            dataset_hash="b" * 64,
            row_count=30,
            role="evaluation_probe",
        )
    with pytest.raises(ValidationError, match="Holdout path is forbidden"):
        ArchiveDatasetBinding(
            dataset_id="btc-holdout",
            path="archive/btc_holdout.parquet",
            dataset_hash="b" * 64,
            row_count=30,
            role="evaluation_probe",
        )
...
def test_dataset_binding_preserves_registered_candidate_fields() -> None:
    packet = build_first_research_candidate_packet()
    manifest = build_archive_dataset_manifest(
    ...
    with pytest.raises(DatasetManifestError, match="Candidate id does not match"):
        bind_candidate_dataset_manifest(
            packet,
            build_archive_dataset_manifest(
                candidate_id="different-candidate",
                dataset_bindings=manifest.dataset_bindings,
                formation_scope=manifest.formation_scope,
                evaluation_scope=manifest.evaluation_scope,
            ),
        )
...
def test_archive_evaluation_requires_all_hash_bindings() -> None:
    packet = build_first_research_candidate_packet()
    manifest = build_archive_dataset_manifest(
        candidate_id=packet.candidate_id,
        dataset_bindings=(
    ...
    assert result.code_hash == "e" * 64
    assert result.policy_hash == "f" * 64
    assert result.parameter_hash == "1" * 64


def test_archive_evaluation_outputs_required_evidence_surfaces() -> None:
    result = _archive_evaluation_result()
...
def test_archive_evaluation_output_remains_no_claim() -> None:
    result = _archive_evaluation_result()
...
def test_research_packet_contains_required_sections() -> None:
    candidate, manifest, evaluation = _archive_evaluation_inputs()
    packet = build_first_research_evidence_packet(
        candidate=candidate,
        manifest=manifest,
        evaluation=evaluation,
        artifact_refs=_research_packet_artifact_refs(),
        project_root=PROJECT_ROOT,
    )
    markdown = packet.to_markdown()
    document = RESEARCH_EVIDENCE_PACKET_DOC.read_text(encoding="utf-8")
...
def test_research_packet_fails_missing_artifact_or_hash(tmp_path: Path) -> None:
    candidate, manifest, evaluation = _archive_evaluation_inputs()
    ...
    with pytest.raises(EvidenceCollectionError, match="Missing evidence artifact"):
        build_first_research_evidence_packet(
            ...
            manifest=manifest,
            evaluation=evaluation,
            artifact_refs=("docs/research/first-packet/MISSING.md",),
            project_root=PROJECT_ROOT,
        )

    broken_evaluation = evaluation.model_copy(update={"code_hash": "PENDING_CODE_HASH"})
    with pytest.raises(FirstResearchEvidencePacketError, match="code_hash must be bound"):
        build_first_research_evidence_packet(
            candidate=candidate,
            manifest=manifest,
            evaluation=broken_evaluation,
            artifact_refs=("docs/research/first-packet/CANDIDATE_PACKET.md",),
            project_root=PROJECT_ROOT,
        )


def test_research_packet_blocks_claim_approvals() -> None:
    candidate, manifest, evaluation = _archive_evaluation_inputs()
    packet = build_first_research_evidence_packet(
        candidate=candidate,
        manifest=manifest,
...
def _archive_evaluation_result():
    _, _, evaluation = _archive_evaluation_inputs()
    return evaluation


def _archive_evaluation_inputs():
    packet = build_first_research_candidate_packet()
    manifest = build_archive_dataset_manifest(
        candidate_id=packet.candidate_id,
        dataset_bindings=(
...
def _research_packet_artifact_refs() -> tuple[str, ...]:
    return (
