...
def test_tasks_records_planned_roadmap_and_active_phase() -> None:
    ...
    assert "Status:     done 2026-05-09" in _task_section(text, "T35")
    ...
    assert "Status:     active" in _task_section(text, "T48")
...
def test_prompt_and_handoff_record_phase7_boundaries() -> None:
    ...
    assert "phase: 11" in prompt.lower()
    ...
    assert "phase: 11 live-feed dry run readiness" in handoff.lower()
