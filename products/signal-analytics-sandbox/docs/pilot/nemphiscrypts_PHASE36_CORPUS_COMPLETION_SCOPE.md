# Phase 36 Scope - nemphiscrypts Corpus Completion

Date: 2026-05-22
Status: `scope_ready_completion_required`
Channel: `nemphiscrypts`
...
| Market candidates | 336 |
| Explicit setup candidates | 18 |
| Position/trade-language candidates | 21 |
| Directional-bias candidates | 226 |
| V1 evaluable claims | 49 |
| V1 confirmed / contradicted | 28 / 21 |
| V1 hit rate | 57.142857% |
| Average directional return | 0.434858% |
| Provider coverage | Binance only: ARB, AVAX, BTC, DOT, ETH, SOL, SUI |
...
Phase 36 has not yet audited `nemphiscrypts` message-ID continuity or full
period coverage. Missing periods/IDs are therefore recorded as
`not_audited_in_phase36_yet`, not as clean coverage.

Required next steps:

1. Build a source coverage manifest from public Telegram `/s/nemphiscrypts`
   captures.
...
mixed-direction rows, and provider gaps before cross-channel scoring.
