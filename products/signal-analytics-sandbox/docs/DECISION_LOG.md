...
| D-008 | 2026-05-07 | Pending (ADR-001 OPEN) | Snapshot persistence: Parquet + JSON metadata, immutable on disk, SHA-256 verified at load — exact format pending research | Reproducibility contract (PSR-2) requires byte-identical re-runs; snapshot immutability is the foundation. Concrete format flag tuning is deferred to ADR-001. | `docs/adr/ADR-001-snapshot-serialization.md`, `docs/IMPLEMENTATION_CONTRACT.md#PSR-5`, `docs/tasks.md#T11` | none |
