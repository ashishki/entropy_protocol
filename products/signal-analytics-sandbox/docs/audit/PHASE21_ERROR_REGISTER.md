...
| P21-E02 | P1 | policy_defined_internal_open_external | media review | `docs/pilot/bablos79_TRANSCRIPT_ACCEPTANCE_POLICY.md` classifies 2 transcript refs as `llm_reviewed_internal`; zero refs are human/operator accepted for external delivery. | Keep refs internal-only unless a human/operator accepts the exact transcript evidence or an explicit claim-level waiver is recorded before external delivery. |
