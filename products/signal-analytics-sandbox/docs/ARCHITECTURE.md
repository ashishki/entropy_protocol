...
| Auto-validation evidence engine | `src/signal_sandbox/auto_validation/` | Phase 40+ subsystem for candidate evidence bundles, validator results, audit logs, strict auto-accept decisions, and customer-facing policy gates. Evidence/result schemas, Phase 41 validators, decision engine, and policy gate are implemented; model review remains triage and deterministic validators own acceptance proof. |
