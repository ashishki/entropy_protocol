...
| Auto-validation evidence engine | `src/signal_sandbox/auto_validation/` | Phase 40+ subsystem for candidate evidence bundles, validator results, audit logs, strict auto-accept decisions, and customer-facing policy gates. Evidence/result schemas are implemented in `evidence.py` and `results.py`; model review remains triage and deterministic validators own acceptance proof. |
